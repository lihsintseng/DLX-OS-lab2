...
void main (int argc, char *argv[])
{
  ...
  for(i = 0; i < 11; i++){
    ...
    if(lock_acquire(l_buffer_lock) != SYNC_SUCCESS){
      Printf("Bad acquire lock s_procs_completed (%d) in ", l_buffer_lock); Printf(argv[0]); Printf(", exiting...\n");
    }
    mc->buffer[mc->head] = message[i];
    Printf("Producer %d inserted: %c\n", Getpid(), message[i]);
    mc->head = (mc->head +1 ) % BUFFERSIZE;
    if(lock_release(l_buffer_lock) != SYNC_SUCCESS){
      Printf("Bad release lock s_procs_completed (%d) in ", l_buffer_lock); Printf(argv[0]); Printf(", exiting...\n");
    }
