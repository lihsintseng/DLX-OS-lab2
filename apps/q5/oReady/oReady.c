...
void main (int argc, char *argv[])
{
 ...
 // uint32 h_mem;          // Handle to the shared memory page
  ...
  if ((c_O = cond_create(l_buffer_lock)) == SYNC_FAIL) {
    Printf("Bad c_O in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
 ...
 // h_mem = dstrtol(argv[1], NULL, 10); // The "10" means base 10
  ...
  #ifdef DEBUG
    Printf("How many child: %d %d\n", -((at->totalO/2-1) ), at->totalN);
  #endif
  if ((s_child_completed = sem_create(-((at->totalO/2-1) ))) == SYNC_FAIL) {
    Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  if ((s_child2_completed = sem_create(-((at->totalO - at->totalO/2-1) ))) == SYNC_FAIL) {
    Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  ...
  for(i = 0; i < at->totalO; i++){
    process_create(MAKE_O2, argv[1], argv[2], s_child_completed_str, s_child2_completed_str, argv[4], c_O_str, NULL);
...
//  Printf("Producer: This is one of the %d producers you created.  \n", at->numprocs);
  ...
  Printf("spawn_me: oReady PID %d is complete.\n", Getpid());
  #endif
