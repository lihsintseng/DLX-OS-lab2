...
//  mc->numprocs = numprocs;
  ...
  for(i=0; i<numprocs; i++) {
    process_create(PRODUCER_TO_RUN, h_mem_str, s_procs_completed_str, l_buffer_lock_str, NULL);
    #ifdef DEBUG
    Printf("Producer %d created\n", i);
    #endif
    process_create(CONSUMER_TO_RUN, h_mem_str, s_procs_completed_str, l_buffer_lock_str, NULL);
    #ifdef DEBUG
