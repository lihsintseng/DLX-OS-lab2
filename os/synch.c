...
static Cond conds[MAX_CONDS]; // All condition variables in tbhe system
...
cond_t CondCreate(lock_t lock) {
  ...
  if(lock==INVALID_LOCK) return INVALID_COND;
...
int CondHandleWait(cond_t c) {
  ...
  RestoreIntrs (intrval);
  ProcessSleep();
  LockHandleAcquire(conds[c].lock);
  return 0;
}
...
int CondHandleSignal(cond_t c) {
  ...
  if (c < 0) return 0;
  if (c >= MAX_CONDS) return 0;
  if (!conds[c].inuse)    return 0;
  
  intrs = DisableIntrs ();

  if(!locks[conds[c].lock].inuse){
    RestoreIntrs (intrs);
    return SYNC_FAIL;
  }
  ...
  dbprintf ('s', "CondHandleSignal: Process %d Signalling on c %d.\n", GetCurrentPid(), (int)(c));
  // Increment internal counter before checking value
  if (!AQueueEmpty(&con->waiting)) { // there is a process to wake up
    ...
    dbprintf ('s', "CondHandleSignal: Waking up PID %d.\n", (int)(GetPidFromAddress(pcb)));
    ProcessWakeup (pcb);
  }else{
    RestoreIntrs (intrs);
    return -2;
  ...
  //LockHandleRelease(conds[c].lock);
  RestoreIntrs (intrs);
...
int CondHandleBroadcast(cond_t c) {
  ...
  while(CondHandleSignal(c) == SYNC_SUCCESS);
  //intrs = DisableIntrs ();

////  if(LockHandleAcquire(conds[c].lock) == SYNC_FAIL) {
  //if(!locks[conds[c].lock].inuse){
  //  RestoreIntrs (intrs);
  //  return SYNC_FAIL;
  //}
